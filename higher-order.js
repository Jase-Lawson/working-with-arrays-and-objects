...
//Code Here
let totalPopulation = populations.reduce(function (acc, element, index, arr) {
  return (acc += element)
}, 0)
...
let bobsTotal = purchases.filter(element => element.owner === 'Bob').map(element => element.price).reduce((sum, element) => sum + element);
